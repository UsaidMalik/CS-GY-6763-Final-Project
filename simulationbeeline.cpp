    ...
    for (int iteration : iterations)
        for(int timeStep : TimeSteps)
            for(int lengthStep : lengthSteps)
             for(bool deterministicFlag : deterministicFlags)
                 for (bool oppositeFlag : oppositeFlags){
                
                        int avg = 0;
                        for (size_t i = 0; i < iteration; ++i){
                            int playerPosition = lengthStep;
                            vector<int> treasureArray(lengthStep*2 +1, 0); // making thing -L and L
                            avg += beelineSimulation(treasureArray, lengthStep, timeStep, deterministicFlag,  oppositeFlag);   
                        }
                        avg /= iteration;
                        cout << iteration << "," << timeStep << "," << lengthStep << "," << avg << "," << deterministicFlag << "," << oppositeFlag << endl;
